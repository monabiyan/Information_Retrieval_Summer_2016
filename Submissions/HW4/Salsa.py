...
while diff > 1:
    ...
    for link in new_graph_outlinks:
        print('ghasem')
        print(diff)
        outlinks=dict.fromkeys(new_graph_outlinks[link], True)

        s=0
        for outlink in outlinks:
            for w in dict.fromkeys(new_graph_inlinks[outlink], True):
                s=s+(float(hub[w])/float((len(new_graph_inlinks[outlink])*len(new_graph_outlinks[w]))))
        hub[link]=s
...
save_dictionary('/Users/mohsennabian/HW4/salsa_hub_dict.txt',hub)
# save_dictionary('/Users/mohsennabian/HW4/salsa_authority_dict.txt',authority)
dictionary_into_2sorted_list_and_save('/Users/mohsennabian/HW4/hub_dict.txt','/Users/mohsennabian/HW4/salsa_hub_demo.txt')
# dictionary_into_2sorted_list_and_save('/Users/mohsennabian/HW4/authority_dict.txt','/Users/mohsennabian/HW4/salsa_authority_demo.txt')
...
while diff>1:
    ...
    for link in new_graph_inlinks:
        print('ali')
        print(diff)
        inlinks = dict.fromkeys(new_graph_inlinks[link], True)
        s=0
        for inlink in inlinks:
            for w in dict.fromkeys(new_graph_outlinks[inlink], True):
                s = s + (float(authority[w]) / float((len(new_graph_outlinks[inlink]) * len(new_graph_inlinks[w]))))

        authority[link]=s
    m=0
    for link in authority:
        # print(hub_old[link])
        m = abs(authority[link] - authority_old[link]) + m
    diff = float(m)
    print(diff)
save_dictionary('/Users/mohsennabian/HW4/salsa_authority_dict.txt',authority)
dictionary_into_2sorted_list_and_save('/Users/mohsennabian/HW4/authority_dict.txt','/Users/mohsennabian/HW4/salsa_authority_demo.txt')
