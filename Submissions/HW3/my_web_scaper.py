...
def Mohsen_relevent_outlinks(PQ_link):
    ...
    for link in all_out_links:
        ...
        try:
            ...
            # status, response = http.request(link)
            tt=time.time()
            if (tt-url_time_visited[url_domain(link)])>1000:

                response = request('GET',link,timeout=10).content
            else :
                time.sleep(1000-tt-url_time_visited[url_domain(link)])
                response = request('GET', link, timeout=10).content
        ...
        try:
            ...
            url_head[link] = soup.title.string
            url_txt[link]=txt
            url_wave_number[link]=url_wave_number[PQ_link]+1
            url_canonical[link]=canonocal(link)

            url_all_outlinks[link]=extract_only_all_outlink(link, soup)
            # url_inlinks[link]=PQ_link
            url_score[link]=2*a+ 15*url_wave_number[link]
            url_domain[link]=extract_only_domain(link)
...
tight_query_list=['terrorism','terrorist','assasinat','bombing','al-Qaeda','massacres','gun violence','jihadi','suicide attack','taliban','Bin Laden','islamic extremist','explosives']
...
while ((len(final_corpus) + len(PQ)) < max_link):
    ...
    if (len(url_txt) > (50)):
        ...
        try:
            path = '/Users/mohsennabian/Infomation_Retrieval_4rdtrial' + '/txt_' + str(index) + '.txt'
            save_dictionary(path, url_txt)
            url_txt={}

            path = '/Users/mohsennabian/Infomation_Retrieval_4rdtrial' + '/outlinks_' + str(index) + '.txt'
            save_dictionary(path, url_outlinks)
            url_outlinks={}

            path = '/Users/mohsennabian/Infomation_Retrieval_4rdtrial' + '/head_' + str(index) + '.txt'
            save_dictionary(path, url_head)
            url_head={}


            path = '/Users/mohsennabian/Infomation_Retrieval_4rdtrial' + '/canonical_' + str(index) + '.txt'
...
try:
    ...
    path_outlinks = '/Users/mohsennabian/Infomation_Retrieval_4rdtrial/outlinks'+ str(index) +'.txt'
    save_dictionary(path_outlinks, url_outlinks)
    url_outlinks={}

    path_inlink = '/Users/mohsennabian/Infomation_Retrieval_4rdtrial/inlinks.txt'
    save_dictionary(path_inlink, url_inlinks)
    url_inlinks={}

    path_final_corpus = '/Users/mohsennabian/Infomation_Retrieval_4rdtrial/final_links.txt'
    save_dictionary(path_final_corpus, final_corpus)
    final_corpus={}

    path_canonical= '/Users/mohsennabian/Infomation_Retrieval_4rdtrial/canonical'+ str(index) +'.txt'
    save_dictionary(path_canonical,url_canonical)
    url_canonical={}

    path_head='/Users/mohsennabian/Infomation_Retrieval_4rdtrial/head'+ str(index) +'.txt'
