...
def query_prep():  # edits the query file and returns a list of edited queries
    ...
    query_list1[4] = 'political motivated hostage'
    query_list1[5] = "military coup etat attempted successful country"
    ...
    query_list1[13] = 'noncommunist industrialized hightech dualuse technology'
    ...
    for query in query_list:
        ...
        for word in query:
            ...
            for i in range(0, len(doc_id)):
                ...
                bm25 = math.log(((D + 0.5) / (total_hit + 0.5)), 10) * (
                (tf_wd + k1 * tf_wd) / (tf_wd + k1 * ((1 - b) + b * len_d / avgs_Docs_length)) * (
                tf_wq + k2 * tf_wq) / (tf_wq + k2))

                laplace_smoothing = math.log((tf_wd + 1) / (len_d + V), 10)  # math.log
                # print('laplace_smoothing',laplace_smoothing)
                Jelinek_smoothing = math.log(((landa * tf_wd) / len_d + (1 - landa) * tf_w_all / sum_length_all_Doc),
                                             10)
        ...
        score_laplace_smoothing_sorted = sorted(score_laplace_smoothing.items(), key=operator.itemgetter(1),
                                                reverse=True)[0:1000]
        ...
        score_Jelinek_smoothing_sorted = sorted(score_Jelinek_smoothing.items(), key=operator.itemgetter(1),
                                                reverse=True)[0:1000]

        all_query_Jelinek_smoothing.append(score_Jelinek_smoothing_sorted)

    return (
    all_query_okapi_tf, all_query_tf_idf, all_query_bm25, all_query_laplace_smoothing, all_query_Jelinek_smoothing)
...
def main():
    ...
    # word_list=read_txt_file('/Users/mohsennabian/Dropbox/Summer_2016/Info_Retrieval/cs6200_Mohsen_Nabian/HW2/in.0.50.txt')
